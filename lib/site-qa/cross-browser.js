...
async function renderIn(engine, url, screensDir) {
  ...
  try {
    ...
    try { shot = path.join(screensDir, `crossbrowser__${engine}.png`); capture = await stableCapture(page, { fullPage: false, path: shot, engine, navigation: nav }); }
    catch (e) { shot = null; }
    await ctx.close();
  } catch (e) { ok = false; navErr = String(e && e.message || e).slice(0, 120); }
  await browser.close();
  return { engine, available: true, ok, navErr, errors: [...new Set(errors)], shot, capture };
}
