...
function recordScan(data) {
  ...
  // PARTIAL RUNS MUST NOT BECOME THE TREND. A run with sections switched off carries FEWER suites,
  // so diffing it against a whole run reports every check of the missing section as newly appeared
  // (or, the other way round, as fixed) — pure fiction generated by a settings toggle, in the one
  // panel people read to decide whether a site got better. Worse, its quality number renormalises
  // over the suites that DID run, so switching off the worst section reads as improvement.
  // The record is still written (it is a real scan and you may want to open it), but it is STAMPED,
  // and loadLatestRecord() will not offer a stamped record as a comparison baseline.
  const partial = !!(data.skipped && data.skipped.suites && data.skipped.suites.length);
  const rec = {
    ...
    partial, skippedSuites: partial ? data.skipped.suites.slice() : [],
  ...
  fs.appendFileSync(path.join(dir, '_index.jsonl'), JSON.stringify({ ts: data.generated, score: data.score, verdict: data.verdict, pass: data.tally.pass, warn: data.tally.warn, fail: data.tally.fail, partial, skippedSuites: rec.skippedSuites, file: path.basename(file) }) + '\n');
  return file;
}
...
// A PARTIAL record is never returned as a baseline: comparing today's whole scan against a prior run
// that had sections switched off invents transitions for every check the old run never made. Walk
// back to the most recent COMPLETE scan instead, and report how many partial runs were stepped over
// so the panel can say the comparison is older than "last scan".
function loadLatestRecord(host) {
  ...
  for (let i = files.length - 1; i >= 0; i--) {
    ...
    if (parsed && parsed.partial) { skippedPartials++; continue; }
    return { data: parsed, source: file, count: files.length, skippedPartials };
  }
  return null;   // every stored scan was partial — there is no honest baseline to compare against
}
