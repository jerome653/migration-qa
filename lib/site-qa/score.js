...
// A row participates in scoring only if its rule carries a real, non-manual deduction.
// `manual` and unregistered rows are inert — they can neither lose nor gain points.
function scorable(row) {
  const d = deductionFor(row);
  return d.points > 0 && row.status !== 'manual';
}
...
function compute(suitesOut) {
  const categories = suitesOut.map(s => {
    ...
    for (const row of s.checks) {
      ...
      const d = deductionFor(row);
      totalRisk += d.points;                       // the worst this check could cost
      if (row.status === 'pass') continue;
      openRisk += d.points;                        // ...and what it actually costs today
      deductions.push({ label: row.name, points: d.points, ruleId: d.ruleId, status: row.status });
    }

    deductions.sort((a, b) => b.points - a.points);

    // No scorable checks → no opinion. score:null is EXCLUDED from the overall below, rather than
    // defaulting to 100 (which would silently inflate the average with a suite that measured nothing).
    const score = totalRisk > 0 ? Math.round(100 * (1 - openRisk / totalRisk)) : null;
