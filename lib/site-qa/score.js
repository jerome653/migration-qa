...
function scopedTotalRisk(suiteKey, excluded) {
  const base = TOTAL_RISK_BY_SUITE[suiteKey] || 0;
  if (!excluded || !excluded.size) return base;
  let off = 0;
  for (const id of excluded) {
    const r = getById(id);
    if (r && r.suite === suiteKey && !r.manual && r.deduction > 0) off += r.deduction;
  }
...
function compute(suitesOut, opts = {}) {
  ...
  const categories = suitesOut.map(s => {
    ...
    const totalRisk = scopedTotalRisk(s.key, excluded);
