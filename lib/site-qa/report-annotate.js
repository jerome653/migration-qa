...
//
// The first cut of this file capped .wrapc at 112mm to force one page x viewport onto a single
// landscape sheet. On the short fixture pages it looked fine. On a real one it was wrong twice
// over: sgen.com's home capture is 1920x14835, so `max-height + width:auto` letterboxed it to a
// 55x423 sliver — unreadable — AND the image then no longer filled .wrapc, so the annotation
// floated across the full pane instead of sitting on its content. Both were caused by the cap.
//
// So: no cap. Panes are width-constrained (image fills the wrapper, marks stay anchored) and a
// sheet is allowed to fragment across PDF pages — each page x viewport still STARTS on a fresh
// page. A tall page therefore costs several sheets of paper, which is the honest price of showing
// all of it; nothing is hidden and no mark is displaced.
const PRINT_CSS = `
body{background:#000}
.wrap{padding:0}
.sheet{break-after:page;page-break-after:always;break-inside:auto;margin:0;border:0;border-radius:0;padding:6mm 4mm}
