...
// SG-Builder breakpoints (default). The audit renders at the exact widths SGB uses so the match
// lines up with how the staging build was authored. Override with opts.viewports if needed.
const VIEWPORTS = [
  { label: '1920 · desktop-xl', width: 1920, height: 1080 },
  { label: '1199 · desktop', width: 1199, height: 900 },
  { label: '991 · tablet-landscape', width: 991, height: 800 },
  { label: '767 · tablet', width: 767, height: 1024 },
  { label: '575 · mobile-landscape', width: 575, height: 800 },
  { label: '480 · mobile', width: 480, height: 900 },
];
...
async function run(refUrl, candUrl, { maxPages = 20, urlMap = null, outDir, viewports = null, log = () => {}, progress = () => {} } = {}) {
  ...
  for (const pr of pairs) {
    const vps = [];
    for (const vp of VPS) {
      const base = `${slug(pr.path)}__${vp.label}`;
      const refShot = path.join(shotsDir, base + '__ref.png');
      const candShot = path.join(shotsDir, base + '__cand.png');
      const diffShot = path.join(shotsDir, base + '__diff.png');
