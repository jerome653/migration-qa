...
// SGEN skin: pure-black background, crisp SGEN red (no pink), warm gold/amber for functional
// pass/warn only (brand palette — NO green, NO blue). Screenshots sit at the top and open in a
// click-through lightbox. Every number/row is real audit data; 'manual' is a neutral status.
...
.ltile{background:var(--surface);border:1px solid var(--line);border-radius:12px;box-shadow:var(--shadow-sm);padding:14px;cursor:pointer;transition:border-color .12s,transform .12s;display:flex;flex-direction:column;gap:8px;text-align:left;font-family:inherit}
.ltile:hover{border-color:var(--brand);transform:translateY(-2px)}
.ltile.new{border-color:var(--brand-solid)}
.ltile .lt-badge{font-family:var(--mono);font-size:8.5px;letter-spacing:.08em;text-transform:uppercase;color:#fff;background:var(--brand-solid);border-radius:5px;padding:1px 6px;align-self:flex-start}
.ltile .lt-top{display:flex;align-items:baseline;justify-content:space-between}
.ltile .lt-nm{font-size:12.5px;font-weight:650}.ltile .lt-sc{font-family:var(--mono);font-size:22px;font-weight:730;line-height:1}
...
(function(){
  ...
  var tiles=order.map(function(k){var v=L[k];return '<button class="ltile'+(v.isNew?' new':'')+'" onclick="lensGo(\\''+k+'\\')">'+(v.isNew?'<span class="lt-badge">New</span>':'')+'<div class="lt-top"><span class="lt-nm">'+esc(v.name)+'</span><span class="lt-sc" style="color:'+col(v.score)+'">'+v.score+'</span></div><div class="lt-trk"><i style="width:'+v.score+'%;background:'+col(v.score)+'"></i></div><span class="lt-sub">'+(v.count?v.count+' issue'+(v.count>1?'s':''):'clean')+'</span></button>';}).join('');
  ...
  host.innerHTML='<div class="lens-h">Inspector lenses \\u2014 the same findings by dimension</div><div class="lens-tiles">'+tiles+'</div><div class="lens-tabs">'+tabs+'</div>'+overview+views;
})();
