...
.bar-in{max-width:1120px;margin:0 auto;padding:11px 22px;display:flex;align-items:center;gap:16px;flex-wrap:wrap}
...
.wrap{max-width:1120px;margin:0 auto;padding:26px 22px 72px}
...
.qs-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(184px,1fr));gap:10px}
...
.dash{margin-bottom:4px}
.dash-row{display:flex;align-items:center;gap:10px;width:100%;text-align:left;background:var(--surface-2);border:1px solid var(--line);border-radius:9px;padding:9px 12px;margin:5px 0;cursor:pointer;color:var(--ink);font-family:var(--sans);font-size:12.5px;transition:border-color .12s ease,transform .12s ease}
.dash-row:hover{border-color:var(--brand);transform:translateX(3px)}
.dash-row .d-st{width:16px;height:16px;border-radius:50%;flex:none;display:grid;place-items:center}.dash-row .d-st svg{width:10px;height:10px}
.dash-row[data-status="fail"] .d-st{background:var(--fail-bg);color:var(--fail)}.dash-row[data-status="warn"] .d-st{background:var(--warn-bg);color:var(--warn)}
.dash-row .d-name{font-weight:600;flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.dash-row .d-suite,.dash-row .d-n{font-family:var(--mono);font-size:10.5px;color:var(--ink-faint);flex:none}
.dash-row .d-sev{font-family:var(--mono);font-size:9.5px;font-weight:700;text-transform:uppercase;border-radius:5px;padding:2px 6px;flex:none}
.dash-row .d-sev.critical,.dash-row .d-sev.high{color:var(--fail);background:var(--fail-bg)}.dash-row .d-sev.medium{color:var(--warn);background:var(--warn-bg)}.dash-row .d-sev.low{color:var(--ink-soft);background:var(--surface)}
.dash-row .d-go{color:var(--ink-faint);flex:none}
.dash-clean{font-family:var(--mono);font-size:12px;color:var(--pass);margin:4px 0 0}
...
// ---- copy-as-markdown: one dev-ready ticket per issue, with page → section → element mapping + evidence ----
function mdItem(c,it){var L=['### QA issue: '+(c.name||'')+(it.id&&it.id!=='(whole page)'?' — \`'+it.id+'\`':'')];
L.push('- **Page:** '+(it.page||'—'));
L.push('- **Where:** '+(it.section&&it.section!=='—'?it.section+' → ':'')+(it.id||'(whole page)'));
if(it.value)L.push('- **Measured:** '+it.value);
if(c.detail)L.push('- **Details:** '+c.detail);
if(c.severity)L.push('- **Severity:** '+c.severity+(c.ruleId?' · Rule '+c.ruleId:''));
if(it.evidence)L.push('- **Evidence screenshot:** \`'+it.evidence+'\` (in the run\\'s screenshots folder — name reads page--section--component--issue--viewport)');
L.push('- **Source:** SGEN Site QA · '+(D.host||'')+' · '+(D.generated||''));
return L.join('\\n');}
function mdCheck(c){var L=['## QA issue: '+(c.name||'')];
L.push('- **Status:** '+c.status.toUpperCase()+(c.severity?' · severity '+c.severity:'')+(c.ruleId?' · Rule '+c.ruleId:''));
if(c.target)L.push('- **Target:** '+c.target);
if(c.detail)L.push('- **Details:** '+c.detail);
var its=(c.items||[]).slice(0,30);
if(its.length){L.push('','| Page | Section | Element | Measured | Evidence |','|---|---|---|---|---|');
its.forEach(function(it){L.push('| '+(it.page||'—')+' | '+(it.section||'—')+' | \`'+(it.id||'')+'\` | '+(it.value||'')+' | '+(it.evidence?'\`'+it.evidence+'\`':'—')+' |');});
if((c.items||[]).length>30)L.push('','+'+((c.items||[]).length-30)+' more occurrences in report.json');}
L.push('','_Evidence filenames read page--section--component--issue--viewport; files sit in the run\\'s screenshots folder._');
L.push('_Source: SGEN Site QA · '+(D.host||'')+' · '+(D.generated||'')+'_');
return L.join('\\n');}
...
function drill(items,c){if(!items||!items.length)return '';var hasEv=items.some(function(it){return it.evidence;});var rows=items.slice(0,500).map(function(it){return '<tr><td class="pg">'+pgLink(it.page)+'</td><td>'+esc(it.section||'—')+'</td><td class="id">'+idLink(it.id||'')+'</td><td class="vl">'+esc(it.value||'')+'</td>'+(hasEv?'<td class="vl">'+(it.evidence?'<span title="'+esc(it.evidence)+'">&#128247;</span>':'—')+'</td>':'')+'<td>'+copyBtn(mdItem(c,it),'\\u29C9')+'</td></tr>';}).join('');var more=items.length>500?'<tr><td colspan="6">+'+(items.length-500)+' more (full list in report.json)</td></tr>':'';return '<details class="drill"><summary>&#9656; show '+items.length+' occurrence'+(items.length>1?'s':'')+' &middot; page &middot; section &middot; identifier</summary><div class="dwrap"><table><thead><tr><th>Page</th><th>Section</th><th>Identifier</th><th>Value</th>'+(hasEv?'<th>Evidence</th>':'')+'<th></th></tr></thead><tbody>'+rows+more+'</tbody></table></div></details>';}

document.getElementById('ov').innerHTML=D.suites.map(function(su){var t=su.pass+su.warn+su.fail+su.manual;function seg(c,n){return n?'<i class="'+c+'" style="width:'+(n/t*100)+'%"></i>':'';}return '<div class="ovi"><div class="top"><b>'+esc(su.name)+'</b><span class="r">'+su.pass+'/'+(su.pass+su.warn+su.fail)+'</span></div><div class="track">'+seg('p',su.pass)+seg('w',su.warn)+seg('f',su.fail)+seg('m',su.manual)+'</div></div>';}).join('');
...
var rows=su.checks.map(function(c,ci){var canCopy=c.status==='fail'||c.status==='warn';return '<div class="row" id="chk-s'+si+'c'+ci+'" data-status="'+c.status+'"><span class="st">'+GLYPH[c.status]+'</span><div class="rbody"><div class="rname">'+esc(c.name)+(canCopy?' '+copyBtn(mdCheck(c),'\\u29C9 Copy for dev'):'')+'</div>'+(c.target?'<div class="rtarget">'+esc(c.target)+'</div>':'')+(c.detail?'<div class="rdetail">'+esc(c.detail)+'</div>':'')+drill(c.items,c)+'</div><span class="rmeta">'+esc(c.meta||'')+'</span></div>';}).join('');
...
// issue dashboard — one clickable row per fail/warn check; click jumps to the full detail row below
(function(){var SEVR={critical:0,high:1,medium:2,low:3};var issues=[];
D.suites.forEach(function(su,si){su.checks.forEach(function(c,ci){if(c.status==='fail'||c.status==='warn')issues.push({su:su.name,si:si,ci:ci,c:c});});});
issues.sort(function(a,b){var s=(a.c.status==='fail'?0:1)-(b.c.status==='fail'?0:1);if(s)return s;return (SEVR[a.c.severity]!=null?SEVR[a.c.severity]:4)-(SEVR[b.c.severity]!=null?SEVR[b.c.severity]:4);});
var el=document.getElementById('dash');if(!el)return;
if(!issues.length){el.innerHTML='<h2>Issue dashboard</h2><p class="dash-clean">No failed or warning checks — nothing to fix.</p>';return;}
el.innerHTML='<h2>Issue dashboard \\u00b7 '+issues.length+' issue'+(issues.length>1?'s':'')+' \\u2014 click a row for full detail + evidence</h2>'+issues.map(function(x){var n=(x.c.items||[]).length;return '<button class="dash-row" data-status="'+x.c.status+'" onclick="gotoIssue(\\'chk-s'+x.si+'c'+x.ci+'\\')"><span class="d-st">'+GLYPH[x.c.status]+'</span><span class="d-name">'+esc(x.c.name)+'</span><span class="d-suite">'+esc(x.su)+'</span>'+(x.c.severity?'<span class="d-sev '+esc(x.c.severity)+'">'+esc(x.c.severity)+'</span>':'')+'<span class="d-n">'+(n?n+' occurrence'+(n>1?'s':''):'')+'</span><span class="d-go">\\u2192</span></button>';}).join('');})();
...
if(D.quality){(function(){function col(s){return s>=90?'var(--pass)':s>=75?'var(--warn)':'var(--fail)';}var cats=D.quality.categories.map(function(c){var deds=c.deductions.length?c.deductions.map(function(d){return '<div class="ded"><b>-'+d.points+'</b>'+esc(d.label)+'</div>';}).join(''):'<div class="clean">clean</div>';return '<div class="qcat"><div class="top"><span class="nm">'+esc(c.name)+'</span><span class="sc" style="color:'+col(c.score)+'">'+c.score+'</span></div><div class="track"><i style="width:'+c.score+'%;background:'+col(c.score)+'"></i></div>'+deds+'</div>';}).join('');document.getElementById('qscore').innerHTML='<div class="qs-head"><h2>SGEN Quality Score</h2><span class="ov" style="color:'+col(D.quality.overall)+'">'+D.quality.overall+'</span></div><div class="qs-grid">'+cats+'</div>';})();}
...
function renderReport(data, outDir) {
  ...
  const shotHtml = shots.length ? `<section class="shotsec"><h2>Rendered screenshots · evidence (${shots.length}) — click a card to open the full page</h2><div class="shotgrid">${shots.slice(0, 80).map(s => `<div class="shotcard" data-cap="${cap(s)}"><div class="thumb"><img loading="lazy" src="${shotSrc(s)}"></div><div class="cardcap"><div class="vp">${esc(String(s.label).replace(/-/g, ' '))}</div><div class="pg">${esc(pgOf(s))}</div></div></div>`).join('')}</div></section>` : '';
...
<div class="wrap">
  <section class="summary">
    <div class="verdict" style="--vc:${vmap[0]};--vcb:${vmap[1]};--vcl:${vmap[2]}">
      ...
      <div class="vtext"><span class="vlabel">${esc(data.verdict)}</span><h1>${esc(failLine)}</h1><p>${graded} automated checks across ${data.suites.length} suites · ${data.tally.manual} item(s) flagged for manual review.</p></div>
    </div>
    <div class="tiles">
  ...
  <section class="strip"><h2>Test suites</h2><div class="ov" id="ov"></div></section>
  ...
  <section class="strip dash" id="dash"></section>
