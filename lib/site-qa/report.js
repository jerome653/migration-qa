...
// Per-section cards LIST their issues inline as clickable rows (owner 2026-07-17: "list the issues
// under each category and make them clickable, not a summary"). Each row deep-links to that issue's
// per-section detail (evidence, every occurrence, element locators) via gotoIssue — the same target
// as the flat Issues section's "detail" link — and mirrors the "Fix first" blocker buttons above, so
// the .qi styling is shared. Capped at CARD_MAX so a pathological page cannot render a 40-row card;
// the overflow chip jumps to the flat Issues section, which owns the full filterable/ignorable list.
var CARD_MAX=8;
var rhtml=rows.slice(0,CARD_MAX).map(function(r){var rn=(r.ch.items||[]).length;return '<button class="qi" data-status="'+r.ch.status+'" onclick="gotoIssue(\\'chk-s'+m.si+'c'+r.ci+'\\')" title="Open this issue in the per-section detail"><span class="d-st">'+GLYPH[r.ch.status]+'</span><span class="qi-name">'+esc(r.ch.name)+'</span><span class="qi-n">'+esc(r.ch.status)+(rn?' \\u00b7 '+rn+'\\u00d7':'')+'</span><span class="d-go">\\u2192</span></button>';}).join('');
if(rows.length>CARD_MAX)rhtml+='<button class="qi qi-more" onclick="gotoIssues()" title="See all '+rows.length+' in the Issues section">+'+(rows.length-CARD_MAX)+' more \\u2192</button>';
...
var head='<div class="qs-head"><h2>Score dashboard \\u00b7 per-section summary \\u00b7 '+totalIssues+' issue'+(totalIssues===1?'':'s')+'</h2>'+(D.quality?'<span class="ov" style="color:'+col(D.quality.overall)+'">'+D.quality.overall+'</span>':'')+'</div><div class="sec-sub">Quality score per section (0\\u2013100) \\u2014 not a pass/fail; the verdict is up top. Every issue is listed in full in the Issues section below.</div>';
