...
document.getElementById('results').innerHTML=D.suites.map(function(su){var t=su.pass+su.warn+su.fail+su.manual;var w=worst(su);var badge=w==='fail'?'<span class="badge fail">'+su.fail+' failed</span>':w==='warn'?'<span class="badge warn">'+su.warn+' warning'+(su.warn>1?'s':'')+'</span>':w==='manual'?'<span class="badge manual">needs review</span>':'<span class="badge pass">all passing</span>';
var rows=su.checks.map(function(c){var canCopy=c.status==='fail'||c.status==='warn';return '<div class="row" data-status="'+c.status+'"><span class="st">'+GLYPH[c.status]+'</span><div class="rbody"><div class="rname">'+esc(c.name)+(canCopy?' '+copyBtn(mdCheck(c),'\\u29C9 Copy for dev'):'')+'</div>'+(c.target?'<div class="rtarget">'+esc(c.target)+'</div>':'')+(c.detail?'<div class="rdetail">'+esc(c.detail)+'</div>':'')+drill(c.items,c)+'</div><span class="rmeta">'+esc(c.meta||'')+'</span></div>';}).join('');
...
function renderReport(data, outDir) {
  ...
  const shots = Object.entries(data.shots || {}).flatMap(([url, arr]) => (arr || []).map(s => ({ url, label: s.label, file: s.file })));
