...
function drill(items){if(!items||!items.length)return '';var rows=items.slice(0,500).map(function(it){return '<tr><td class="pg">'+pgLink(it.page)+'</td><td>'+esc(it.section||'—')+'</td><td class="id">'+idLink(it.id||'')+'</td><td class="vl">'+esc(it.value||'')+'</td></tr>';}).join('');var more=items.length>500?'<tr><td colspan="4">+'+(items.length-500)+' more (full list in report.json)</td></tr>':'';return '<details class="drill"><summary>&#9656; show '+items.length+' occurrence'+(items.length>1?'s':'')+' &middot; page &middot; section &middot; identifier</summary><div class="dwrap"><table><thead><tr><th>Page</th><th>Section</th><th>Identifier</th><th>Value</th></tr></thead><tbody>'+rows+more+'</tbody></table></div></details>';}
...
var rows=su.checks.map(function(c){return '<div class="row" data-status="'+c.status+'"><span class="st">'+GLYPH[c.status]+'</span><div class="rbody"><div class="rname">'+esc(c.name)+'</div>'+(c.target?'<div class="rtarget">'+esc(c.target)+'</div>':'')+(c.detail?'<div class="rdetail">'+esc(c.detail)+'</div>':'')+drill(c.items)+'</div><span class="rmeta">'+esc(c.meta||'')+'</span></div>';}).join('');
