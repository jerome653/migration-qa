...
function itemsOf(finds) {
  ...
  for (const f of finds) {
    ...
    if (f.items && f.items.length) f.items.forEach(it => out.push({ page: f.location || '—', section: it.section || '—', id: it.id || '(element)', value: it.value == null ? '' : String(it.value), evidence: ev }));
