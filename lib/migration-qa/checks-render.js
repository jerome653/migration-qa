  ...
  for (const url of targets) {
    ...
    for (let i = 0; i < VIEWPORTS.length; i++) {
      ...
      try {
        ...
        for (const r of res) findings.push({ ...F(SWEEP_RULE[r.check], r.check, r.check === 'low-contrast' ? '7 Accessibility' : '4 Responsive', `${r.detail}${r.selector ? ` — ${r.selector}` : ''} [${v.label}]`, url, r.value), items: [{ id: r.selector || '(element)', section: r.section || '—', value: `${r.value}${r.detail ? ' · ' + r.detail : ''} [${v.label}]` }] });
        const file = path.join(screensDir, `${slug(url)}__${v.label}.png`);
        // full-page (not just hero): Playwright scrolls the whole page, which also forces lazy-loaded
        // assets to load — so below-the-fold images/404s surface in the network + broken-asset checks.
        await page.screenshot({ path: file, fullPage: true });
        shots[url].push({ label: v.label, file });
