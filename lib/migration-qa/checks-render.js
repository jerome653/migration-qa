...
async function renderPass(pageUrls, { screensDir, sampleN = 12, viewports = null, log = () => {}, progress = () => {} } = {}) {
  ...
  // Viewport picker: filter the device×width matrix to the requested labels (mirrors visual-match's
  // opts.viewports). Absent / empty / no-known-label ⇒ the full VIEWPORTS matrix (default unchanged).
  ...
  for (const url of targets) {
    ...
    for (let i = 0; i < activeViewports.length; i++) {
      const v = activeViewports[i];
      try {
        await page.setViewportSize({ width: v.width, height: v.height });
        await page.waitForTimeout(350);
        await page.evaluate(`window.__runContrast = ${v.label === contrastLabel};`);
...
module.exports = { renderPass, VIEWPORTS, FONT_RULE };
